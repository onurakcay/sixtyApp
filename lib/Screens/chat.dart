...
class _SohbetState extends State<Sohbet> {
  ...
  Future<bool> _onback() async {
    bool result = await PlatformBasedAlertDialog(
      title: "Dikkat!",
      content: "Süre dolmadan konuşmadan ayrılırsan eşleşme gerçekleşmez.",
      okButtonText: "Konuşmadan ayrıl",
      cancelText: "İptal",
    ).goster(context);
    print("RESULT: " + result.toString());
    if (result) {
      ...
      MyUserClass _chattingUser = widget.chattingUser;
      Navigator.pop(context);
      return await _userModel.removeMatch(
          _chattingUser.userID, _currentUser.userID);
    } else {
      return false;
    }
  }
